...
leftMargin=10
AteDB=MyDB.SQLiteDB()
Table_Record='TestRecord_PEL3K'
Table_TestLog='TestLog_PEL3K'
...
PSW=MyInstr.DC_PS()
PSW_H_A=MyInstr.DC_PS()
PSW_H_V=MyInstr.DC_PS()
    ...
    def showTestData(self): 
        ...
        try:          
            ...
            query_str = f"""select ItemNo as No_1, SubItemNo as No_2, TestPointNo as No_3 
                ...
                from {Table_Record} where TestDocNo='{self.TestDocNo}' """
...
class MainTest(QtWidgets.QMdiSubWindow, Ui_frmMainTest):#UI 測試項目請在這修改and Data欄位Item Name也須修改一樣   #UI 治具條件列表請修改Data欄位Cond_Fixture     
            ...
            if TNo !='' and self.cboScript.currentText()!='' and self.DBPth!='':                
                ...
                if len(rsScr)>0:
                    ...
                    if ScriptOK==False:
                        ...
                        sSql=f'''insert into {Table_Record} (TestDocNo, ScriptName, ScriptVersion, ItemNo, SubItemNo, TestPointNo, ItemNo, ItemName, ItemName_2t, ItemName_3t, CondName_1, Cond_1, CondUnit_1, 
        ...
        try:  
            ...
            sSql=f"""SELECT A.TestDocNo, A.TestResult, A.Workstation, A.Staff, B.PASS, B.FAIL, B.其他, B.小計 
                    FROM TestSummary A LEFT JOIN (
                        ...
                        FROM {Table_Record} GROUP BY TestDocNo ) B ON A.TestDocNo = B.TestDocNo
    ...
    def getTestItems(self):
        ...
        try:
            AteDB.ConnectSQLite(self.DBPth)
            TDocNo=self.cboReportNo.currentText()
            sSql=f'select distinct ItemNo, ItemName, ScriptName from {Table_Record} where TestDocNo=\'{TDocNo}\' order by ItemNo'            
    ...
    def getFixCond(self):
        ...
        try:            
            AteDB.ConnectSQLite(self.DBPth)
            TDocNo=self.cboReportNo.currentText()
            sSql=f'select distinct Cond_Fixture from {Table_Record} where TestDocNo=\'{TDocNo}\''
            rsFix=AteDB.Query(sSql)        
            AteDB.CloseConnection()            
            j=len(rsFix)-1
            while j>=0:
    ...
    def getSNDocNo(self):
        ...
        try:
            AteDB.ConnectSQLite(self.DBPth)            
            sSql=f"select distinct TestDocNo from {Table_Record} where TestDocNo like '{SerNo}_%' order by TestDocNo desc"   
            print(sSql)           
    ...
    def checkDBField(self): 
        try:       
            if self.db.open():
                query=QSqlQuery(self.db)
                self.Table_Add_Colum(f"{Table_Record}","Datetime_Upload","Datetime")
                ...
                self.Table_Add_Colum(f"{Table_Record}","ScriptVersion", "Text")               
    ...
    def UploadTestData(self): 
        # --------------測試時在開發電腦不上傳-------------
        if fnmatch(self.MyHostName,'lorenzo*') or fnmatch(self.MyHostName,'g0255*'):
            self.txtMsg_append(f'程式開發電腦,不上傳資料.',QColorConstants.Magenta)
            return
        # ---------------------------------
        try:
            self.txtMsg_append(f"{QDateTime.currentDateTime().toString('hh:mm:ss')} 準備上傳測試記錄...") 
            now_str = QDateTime.currentDateTime().toString(self.MyDTFormat)
            self.ledDataUpload.setVisible(True)
            strSQL=''
            TDocNo=''
            sqlite_conn = sqlite3.connect(self.DBPth)
            ...
            mariadb_conn = pymysql.connect(
                ...
                database=self.MariaDB_Name, 
                autocommit=False)
            self.txtMsg_append(f"{QDateTime.currentDateTime().toString('hh:mm:ss')} 連線到雲端資料庫{self.MariaDB_Host}.")            
            sqlite_cur = sqlite_conn.cursor()
            mariadb_cur = mariadb_conn.cursor()
            # 1. 找出 Datetime_Upload IS NULL 的 TestRecrod 資料
            strSQL=f'''SELECT TDatetime, TestDocNo, ScriptName, ScriptVersion ,ItemNo, SubItemNo, TestPointNo, ItemName, 
                    ...
                    FROM {Table_Record} WHERE Datetime_Upload IS NULL            
                    '''
            col_TestDocNo=1         #第2欄是 TestDocNo
            col_Datetime_Upload=-1  #最後1欄是 Datetime_Upload
            sqlite_cur.execute(strSQL)
            testrecrod_rows = sqlite_cur.fetchall()
            if not testrecrod_rows:
                self.txtMsg_append(f"{QDateTime.currentDateTime().toString('hh:mm:ss')} 沒有測試記錄資料需要上傳.")
                # sqlite_conn.close()
                # mariadb_conn.close()
                # return
                raise ValueError("")
            # 2. 找出受影響的 TestDocNo
            doc_nos = list({row[col_TestDocNo] for row in testrecrod_rows})  # 第2欄是 TestDocNo
            self.txtMsg_append(f"{QDateTime.currentDateTime().toString('hh:mm:ss')} 共 {len(doc_nos)} 份 測試文件需要同步上傳, 上傳進行中...")
            # 3. 先把同 TestDocNo 其他 Datetime_Upload 不為 NULL 的也設成 NULL
            sqlite_cur.executemany(f"""
                UPDATE {Table_Record} 
                SET Datetime_Upload = NULL
                WHERE TestDocNo = ?
            """, [(doc_no,) for doc_no in doc_nos])
            sqlite_conn.commit()
            # 4. 重新抓取對應的 TestRecrod 資料
            strSQL=f"""SELECT TDatetime, TestDocNo, ScriptName, ScriptVersion ,ItemNo, SubItemNo, TestPointNo, ItemName, ItemName_2t, ItemName_3t, 
                    CondName_1, Cond_1, CondUnit_1, CondName_2, Cond_2, CondUnit_2, CondName_3, Cond_3, CondUnit_3, Expect, 
                    ExpectUnit, LowLimit, UpLimit, Cond_DUT, Cond_Equip, Measure_Main, Measure_1, Measure_2, Deviation, Deviation_Expect, 
                    Deviation_LowLimit, Deviation_UpLimit, Judge, Comment, Cond_Fixture, Cond_Fiixture_Other, Datetime_Upload 
                    FROM {Table_Record} 
                    WHERE TestDocNo IN ({','.join(['?'] * len(doc_nos))})            
                    """
            sqlite_cur.execute(strSQL, doc_nos)
            testrecrod_rows_raw = sqlite_cur.fetchall()
            #將TestRecord數值欄位空字串值轉為None
            testrecrod_rows=[]
            for row in testrecrod_rows_raw:
                testrecrod_rows.append(list(row))            
            for row in testrecrod_rows:
                if str(row[19]).strip()=="":
                    row[19]=None
                if str(row[21]).strip()=="":
                    row[21]=None
            ...
            # 5. 抓取 TestSummary 資料
            strSQL=f"""
                SELECT TestDocNo, ScriptName, ScriptVersion, Workstation, CreateDatetime, LastTestDatetime, TestResult, 
                    DUTModel, DUTSerNo, DUTIDN, Temperature, Humidity, EquipList, Staff, Datetime_Upload 
                    FROM TestSummary  
                    WHERE TestDocNo IN ({','.join(['?'] * len(doc_nos))})
            """
            col_TestDocNo_Summary=0             # 第1欄是 TestDocNo
            col_Datetime_Upload_Summary=-1      #Datetime_Upload在最後1欄

            sqlite_cur.execute(strSQL, doc_nos)
            testsummary_rows = sqlite_cur.fetchall()            
            
            # 6. MariaDB 交易開始

            upload_failed_Doc_qty=0
            for idx, doc_no in enumerate(doc_nos, start=1):
                try:
                    mariadb_cur.execute("START TRANSACTION")        #MariaDB 交易開始
                    TDocNo=doc_no
                    # 進度顯示
                    self.ledDataUpload.setText(f"[{idx}/{len(doc_nos)}] 測試文件 {doc_no} 上傳進行中...")
                    QApplication.processEvents()
                    # 刪除 MariaDB 舊資料
                    strSQL=f"DELETE FROM {Table_Record} WHERE TestDocNo = %s"
                    mariadb_cur.execute(strSQL, (doc_no,))
                    strSQL=f"DELETE FROM TestSummary WHERE TestDocNo = %s"
                    mariadb_cur.execute(strSQL, (doc_no,))
                    # 批次插入 TestRecrod
                    recrod_batch = [list(row) for row in testrecrod_rows if row[col_TestDocNo] == doc_no]   #第2欄是 TestDocNo
                    ri=1
                    for row in recrod_batch:
                        row[col_Datetime_Upload] = now_str  # Datetime_Upload 在最後一欄,插入日期時間
                       
                        
                    if recrod_batch:
                        strSQL=f"""
                            INSERT INTO {Table_Record} 
                            (TDatetime, TestDocNo, ScriptName, ScriptVersion ,ItemNo, SubItemNo, TestPointNo, ItemName, ItemName_2t, ItemName_3t, 
                            CondName_1, Cond_1, CondUnit_1, CondName_2, Cond_2, CondUnit_2, CondName_3, Cond_3, CondUnit_3, Expect, 
                            ExpectUnit, LowLimit, UpLimit, Cond_DUT, Cond_Equip, Measure_Main, Measure_1, Measure_2, Deviation, Deviation_Expect, 
                            Deviation_LowLimit, Deviation_UpLimit, Judge, Comment, Cond_Fixture, Cond_Fiixture_Other, Datetime_Upload)
                            ...
                            %s, %s, %s, %s, %s, %s, %s)
                        """
                        mariadb_cur.executemany(strSQL, recrod_batch)

                    # 批次插入 TestSummary
                    summary_batch = [list(row) for row in testsummary_rows if row[col_TestDocNo_Summary] == doc_no]
                    for row in summary_batch:
                        row[col_Datetime_Upload_Summary] = now_str  # 假設 Datetime_Upload 在最後一欄
                    if summary_batch:
                        strSQL=f"""
                            INSERT INTO TestSummary 
                            (TestDocNo, ScriptName, ScriptVersion, Workstation, CreateDatetime, LastTestDatetime, TestResult, DUTModel, DUTSerNo, DUTIDN, 
                            Temperature, Humidity, EquipList, Staff, Datetime_Upload)
                            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s,
                            %s, %s, %s, %s, %s)
                        """
                        mariadb_cur.executemany(strSQL, summary_batch)
                    # 進度顯示
                    self.ledDataUpload.setText(f"[{idx}/{len(doc_nos)}] 測試文件 {doc_no} 已上傳完成")
                    # 7. MariaDB 提交
                    mariadb_conn.commit()
                    # 8. 回寫 SQLite 的 Datetime_Upload
                    # strSQL=f"""
                    #     UPDATE {Table_Record}
                    #     SET Datetime_Upload = ?
                    #     WHERE TestDocNo IN ({','.join(['?'] * len(doc_nos))})
                    # """
                    # sqlite_cur.execute(strSQL, [now_str] + doc_nos)
                    
                    strSQL=f"""
                        UPDATE {Table_Record}
                        SET Datetime_Upload = '{now_str}'
                        WHERE TestDocNo = '{doc_no}'
                    """            
                    sqlite_cur.execute(strSQL)
                    ...
                    if  str(e)!="":
                        self.txtMsg_append(f"{inspect.currentframe().f_code.co_name} Error: \n{str(e)}",QColorConstants.Red)
                        self.txtMsg_append(f'錯誤時的測試文件編號:{TDocNo}',QColorConstants.Magenta)
                        self.txtMsg_append(f'錯誤時的SQL:\n{strSQL}',QColorConstants.Magenta)
                        self.txtMsg_append(f"上傳{doc_no}, TestRecord,Testsummary失敗 ❌，進行 rollback", QColorConstants.Magenta)
                        mariadb_conn.rollback()
                        upload_failed_Doc_qty+=1
                QApplication.processEvents()
            if upload_failed_Doc_qty==0:
                self.txtMsg_append(f"{QDateTime.currentDateTime().toString('hh:mm:ss')} 上傳測試記錄完成：TestRecrod {len(testrecrod_rows)} 筆，TestSummary {len(testsummary_rows)} 筆 ✅",QColorConstants.Blue)
            else:
                self.txtMsg_append(f"{QDateTime.currentDateTime().toString('hh:mm:ss')} 預計上傳測試文件{len(doc_nos)}份, 上傳失敗:{upload_failed_Doc_qty}份",QColorConstants.Magenta)
            ...
            if  str(e)!="":
                ...
                self.txtMsg_append(f'錯誤時的測試文件編號:{TDocNo}',QColorConstants.Magenta)
                self.txtMsg_append(f'錯誤時的SQL:\n{strSQL}',QColorConstants.Magenta)
                # self.txtMsg_append(f"上傳TestRecord,Testsummary失敗 ❌，進行 rollback", QColorConstants.Magenta)
                # mariadb_conn.rollback()            
        finally:            
            QApplication.processEvents()
        ...
        try:
            # 2. 撈出未上傳資料
            strSQL=f"""
                SELECT TestDocNo, ItemNo, SubItemNo, TestPointNo, Workstation, Staff, Measure_Main,  Measure_1, Measure_2, Judge, 
                Comment,Datetime_Start, Datetime_End, Datetime_Upload 
                FROM TestLog WHERE Datetime_Upload IS NULL
            """
            sqlite_cur.execute(strSQL)
            rows_raw = sqlite_cur.fetchall()           

            if not rows_raw:
                self.txtMsg_append(f"{QDateTime.currentDateTime().toString('hh:mm:ss')} 沒有TestLog資料需要上傳.")
                raise ValueError("")

            self.txtMsg_append(f"{QDateTime.currentDateTime().toString('hh:mm:ss')} 即將上傳 {len(rows_raw)} 筆 TestLog 資料...")
            col_Datetime_Upload=-1     #Datetime_Upload在最後一欄
            #將TestRecord數值欄位空字串值轉為None
            rows=[]
            for row in rows_raw:
            ...
            for row in rows:
                ...
                if str(row[8]).strip()=="":
                    row[8]=None
                row[col_Datetime_Upload] = now_str
            # 3. 整理 doc_key 組合鍵，用來刪除
            doc_keys = list(set((row[0], row[11]) for row in rows))  # (TestDocNo, Datetime_Start)
            # 4. 開啟 MariaDB 交易
            # mariadb_conn.begin()
            mariadb_cur.execute("START TRANSACTION")
            # 5. 刪除舊資料
            strSQL=f"""
                DELETE FROM {Table_TestLog} WHERE TestDocNo = %s AND Datetime_Start = %s
            """
            mariadb_cur.executemany(strSQL, doc_keys)

            # 6. 插入新資料（加上 Datetime_Upload）
            # insert_rows = [list(row) + [now_str] for row in rows]            
            strSQL=f"""
                INSERT INTO {Table_TestLog} 
                (TestDocNo, ItemNo, SubItemNo, TestPointNo, Workstation, Staff, Measure_Main,  Measure_1, Measure_2, Judge, 
                Comment, Datetime_Start, Datetime_End, Datetime_Upload)
                VALUES (%s, %s, %s, %s, %s,%s, %s, %s, %s, %s,%s, %s, %s, %s)
            """
            mariadb_cur.executemany(strSQL, rows)

            # 7. 回寫 SQLite 的 Datetime_Upload
            sqlite_conn.execute("BEGIN")
            strSQL=f"""
                UPDATE TestLog
                SET Datetime_Upload = ?
                WHERE TestDocNo = ? AND Datetime_Start = ?
            """
            sqlite_cur.executemany(strSQL, [(now_str, row[0], row[11]) for row in rows])

            # 8. Commit
            mariadb_conn.commit()
            sqlite_conn.commit()
            self.txtMsg_append(f"{QDateTime.currentDateTime().toString('hh:mm:ss')} 上傳TestLog成功 ✅",QColorConstants.Blue)
            ...
            if str(e) !="":
                ...
                self.txtMsg_append(f"{QDateTime.currentDateTime().toString('hh:mm:ss')} 上傳TestLog失敗 ❌",QColorConstants.Red)
                self.txtMsg_append(f"{QDateTime.currentDateTime().toString('hh:mm:ss')} {inspect.currentframe().f_code.co_name} Error: \n{str(e)}",QColorConstants.Red)
                self.txtMsg_append(f'錯誤時的SQL:\n{strSQL}',QColorConstants.Magenta)
        ...
        sqlite_conn.close()
        mariadb_conn.close()
        self.ledDataUpload.setVisible(False)
        ...
        try:       
            for i in range(0,self.model.rowCount()):            
                ...
                if rdev.value('Name')=='待測機_RS232':   
                    ...
                    dev=PSW
                elif rdev.value('Name')=='直流電源_H_A':
                    dev=PSW_H_A
                elif rdev.value('Name')=='直流電源800V':
                    dev=PSW_H_V
    ...
    def onStartClick(self):
        ...
        try:     
            ...
            #------------------------------------------------------------------------------------------------------------
            if fnmatch(self.cboScript.currentText(),'PEL-3*')or fnmatch(self.cboScript.currentText(),'LSG*'): #依腳本, 設定DUT物件
    ...
    def Blank_TRecord(self, r=QSqlQueryModel.record):
        ...
        #r.setValue(TH.col_Comment,'')  為了使用備註欄為把清空表單這項關掉變註解.
    ...
    def Update_TRecord(self, r=QSqlQueryModel.record,WriteLog=True,StartDateTime=''):
        ...
        try:
            ...
            sSql=f"update {Table_Record}  set "
            ...
            if r.value(TH.col_Measure_Main)!="":
                ...
                Meas_M='NULL'
            if r.value(TH.col_UpLimit)!="":
                sSql+=f", UpLimit={r.value(TH.col_UpLimit)}" 
                Up_Lim=r.value(TH.col_UpLimit)
            else:
                sSql+=f", UpLimit=NULL"
                Up_Lim='NULL'
            if r.value(TH.col_LowLimit)!="":
                sSql+=f", LowLimit={r.value(TH.col_LowLimit)}"
                Low_Lim=r.value(TH.col_LowLimit)
            else:
                sSql+=f", LowLimit=NULL"
                Low_Lim='NULL'
            ...
            if WriteLog:
                ...
                sSql=f"""Insert into TestLog
                    ...
                    Workstation, Staff, 
                    Measure_Main, Deviation_LowLimit, Deviation_UpLimit, Measure_1, Measure_2, Judge,
                    ...
                    (
                        ...
                        {Meas_M}, {DEV_Low_Lim}, {DEV_Up_Lim}, {Meas_1}, {Meas_2}, '{rJudge}',  
                        '{StartDateTime}', '{rdt}'
    ...
    #PSU選擇哪一台
    def select_psw(self, cond_dut):     
        value = float(cond_dut)
        if value > 21.6:
            return PSW_H_A
        elif value > 2.9:
            return PSW
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                try:
                    ...
                    print(f"DUT IDN: {idn}")
                    if "PEL-3111AH" in idn:
                        delay_time = 1.8
                        self.txtMsg_append(f"偵測到 PEL-3111AH,delay 改為 1.8 秒")
                except Exception as e:
                    print(f"讀取 IDN 失敗，使用預設 delay: {e}")
                PCS_read=PCS.getReading_DCI(delay_before=delay_time)
                #PCS_read=PCS.getReading_DCI(delay_before=0.9)
                self.txtMsg_append(f"PCS 取值:\n{PCS_read}")
                self.PSC_CCACC.append(PCS_read)
                print(self.PSC_CCACC)
                DMM_CC_MEAS_V=DMM.getMainReading(delay_before=0.5)
                ...
                COMMON_3021     =   {'LOW': 0.35, 'MIDDLE':35, 'HIGH': 35,  'VAR_1': 500000,  'MODE': 'A'}
                COMMON_3041     =   {'LOW': 0.7, 'MIDDLE':70,  'HIGH': 70,  'VAR_1': 500000,  'MODE': 'A'}
                COMMON_3111     =   {'LOW': 2.1, 'MIDDLE':210, 'HIGH': 210, 'VAR_1': 500000,  'MODE': 'A'}
                COMMON_3021H    =   {'LOW': 87.5,'MIDDLE':8.75,'HIGH': 8.75, 'VAR_1': 3240000, 'VAR_2': 2210000,'MODE': 'B'}
                COMMON_3041H    =   {'LOW': 175, 'MIDDLE':17.5, 'HIGH': 17.5,  'VAR_1': 2210000, 'MODE': 'C'}
                COMMON_3111H    =   {'LOW': 0.525,'MIDDLE':52.5,'HIGH':52.5,  'VAR_1': 2210000, 'MODE': 'D'}
                COMMON_3031AE   =   {'LOW': 60,'MIDDLE':60,     'HIGH':60,  'VAR_1': 500000,  'MODE': 'D'}
                COMMON_3032AE   =   {'LOW': 15,'MIDDLE':15,     'HIGH': 15,  'VAR_1': 500000,  'MODE': 'D'}
                model_config = {
                                'PEL-3021A': COMMON_3021,'PEL-3021': COMMON_3021, 'LSG-175A':COMMON_3021,'PEL-73021':COMMON_3021,'CL-3017L':COMMON_3021,'DL-3017L':COMMON_3021,'IEL-3021':COMMON_3021, 
                                'PEL-3041A': COMMON_3041,'PEL-3041': COMMON_3041,'PEL-73041': COMMON_3041,'CL-3035L': COMMON_3041,'DL-3035L': COMMON_3041,'IEL-3041': COMMON_3041,'LSG-350A': COMMON_3041,
                                'PEL-3111A': COMMON_3111,'PEL-3111': COMMON_3111,'PEL-73111': COMMON_3111,'CL-3105L': COMMON_3111,'DL-3105L': COMMON_3111,'LSG-1050A': COMMON_3111, 
                                'PEL-3021H': COMMON_3021H,'PEL-3021AH': COMMON_3021H,'LSG-175AH': COMMON_3021H,  
                                'PEL-3041H': COMMON_3041H,'PEL-3041AH': COMMON_3041H,'LSG-350AH': COMMON_3041H, 
    ...
    def PEL_CC_Mea_Readback(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                self.DUT.setInput_State(1)
                self.txtMsg_append(f"DUT_LOAD_ON...")
                PCS_CCMEAS_READBK=PCS.getReading_DCI(delay_before=1)
                PEL_MEAS_READBK=self.DUT.getMeasure_Current(delay_before=0.5)
                PEL_CC_SET_V=self.DUT.getCC_Current()

                COMMON_3021     =   {'LOW': 0.35, 'MIDDLE':35, 'HIGH': 35,   'MODE': 'A'}
                COMMON_3041     =   {'LOW': 0.7, 'MIDDLE':70,  'HIGH': 70,   'MODE': 'A'}
                COMMON_3111     =   {'LOW': 2.1, 'MIDDLE':210, 'HIGH': 210,  'MODE': 'A'}
                COMMON_3021H    =   {'LOW': 87.5,'MIDDLE':8.75,'HIGH': 8.75, 'MODE': 'A'}
                COMMON_3041H    =   {'LOW': 175, 'MIDDLE':17.5, 'HIGH': 17.5, 'MODE': 'A'}
                COMMON_3111H    =   {'LOW': 0.525,'MIDDLE':52.5,'HIGH':52.5, 'MODE': 'A'}
                COMMON_3031AE   =   {'LOW': 60,   'MIDDLE':60,     'HIGH':60,   'MODE': 'B'}
                COMMON_3032AE   =   {'LOW': 15,   'MIDDLE':15,     'HIGH': 15,  'MODE': 'B'}

                model_config = {
                                'PEL-3021A': COMMON_3021,'PEL-3021': COMMON_3021,'LSG-175A':COMMON_3021,'PEL-73021':COMMON_3021,'CL-3017L':COMMON_3021,'DL-3017L':COMMON_3021,'IEL-3021':COMMON_3021,
                                'PEL-3041A': COMMON_3041,'PEL-3041': COMMON_3041,'PEL-73041': COMMON_3041,'CL-3035L': COMMON_3041,'DL-3035L': COMMON_3041,'IEL-3041': COMMON_3041,'LSG-350A': COMMON_3041,
                                'PEL-3111A': COMMON_3111,'PEL-3111': COMMON_3111,'CL-3105L': COMMON_3111,'DL-3105L': COMMON_3111,'LSG-1050A': COMMON_3111,
                                'PEL-3021H': COMMON_3021H,'PEL-3021AH': COMMON_3021H,'LSG-175AH': COMMON_3021H,
                                'PEL-3041H':  COMMON_3041H,'PEL-3041AH':  COMMON_3041H,'LSG-350AH':  COMMON_3041H,
                                'PEL-3111H':  COMMON_3111H,'PEL-3111AH':  COMMON_3111H,'LSG-1050AH':  COMMON_3111H,
                                'PEL-3031AE': COMMON_3031AE,
    ...
    def PEL_CR_Accuracy(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                psw.setOutput_State(1,delay_after=0.5)
                self.DUT.setInput_State(1)
                self.txtMsg_append(f"DUT_LOAD_...")
                PCS_CRMEAS=PCS.getReading_DCI(delay_before=1)
                self.txtMsg_append(f"PCS 取值:\n{PCS_CRMEAS}")
                DMM_CR_MEAS_V=DMM.getMainReading(delay_before=1.5)
                PEL_CR_SET = self.DUT.getCR_Resistance()
                
                COMMON_3021  = {"LOW": 0.35, "HIGH": 35, "V_DEN": 500000}
                COMMON_3021H = {"LOW": 87.5, "HIGH": 8.75, "V_DEN": 3240000}
                COMMON_3041  = {"LOW": 0.7, "HIGH": 70, "V_DEN": 500000}
                COMMON_3041H = {"LOW": 175, "HIGH": 17.5, "V_DEN": 2210000}
                COMMON_3111  = {"LOW": 2.1, "HIGH": 210, "V_DEN": 500000}
                COMMON_3111H = {"LOW": 0.525, "HIGH": 52.5, "V_DEN": 2210000}
                COMMON_3031E = {"LOW": 60, "HIGH": 60, "V_DEN": 0}
                COMMON_3032E = {"LOW": 6, "HIGH": 6 , "V_DEN": 0}
                # FS 參數表
                FS_TABLE = {
                    "PEL-3021"  : COMMON_3021,"PEL-3021A"  : COMMON_3021,"LSG-175A": COMMON_3021,
                    "PEL-3021H" : COMMON_3021H, "LSG-175AH" :COMMON_3021H,
                ...
                if model and oper_mode in FS_TABLE[model]:
                    ...
                    if oper_mode == "LOW":
                        if model in ('PEL-3111H','LSG-1050AH','PEL-3111AH'):
                            ...
                            RL = PEL_CR_STANDARD - ((0.005 * PEL_CR_STANDARD) + (0.005 * FS)) - (DMM_CR_MEAS_V / V_DEN)
                            RU = PEL_CR_STANDARD + ((0.005 * PEL_CR_STANDARD) + (0.005 * FS)) + (DMM_CR_MEAS_V / V_DEN)
                    elif model in ['PEL-3021H', 'PEL-3041H','PEL-3021AH','PEL3041AH','LSG-350AH','LSG-175AH']:
                        PEL_CR_STANDARD = DMM_CR_MEAS_V * PEL_CR_RE / 1000
                        RL = PEL_CR_STANDARD - ((0.005 * PEL_CR_STANDARD) + (0.005 * FS)) + (DMM_CR_MEAS_V / V_DEN)
                        RU = PEL_CR_STANDARD + ((0.005 * PEL_CR_STANDARD) + (0.005 * FS)) + (DMM_CR_MEAS_V / V_DEN)
                    elif model.endswith("E"):
                ...
                if strIName=='CR Accuracy':
                    if model.endswith("E"):
                        ...
                        r.setValue(TH.col_Measure_2,DMM_CR_MEAS_V)
    ...
    def PEL_CV_Accuracy(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                self.txtMsg_append(f"DMM_取值:\n{DMM_CV_MEAS}")
                PEL_CV_SET_V = self.DUT.getCV_Voltage()
                
                COMMON_3021     =  {'LOW': 15, 'MID':35, 'HIGH': 150,    'MODE': 'A'}
                COMMON_3041     =  {'LOW': 15, 'MID':70,  'HIGH': 150,   'MODE': 'A'}
                COMMON_3111     =  {'LOW': 15, 'MID':210, 'HIGH': 150,   'MODE': 'A'}
                COMMON_3021H    =  {'LOW': 80,'MID':8.75,    'HIGH': 800,    'MODE': 'A'}
                COMMON_3041H    =  {'LOW': 80, 'MID':17.5,   'HIGH':800,   'MODE': 'A'}
                COMMON_3111H    =  {'LOW': 80,'MID':52.5,    'HIGH':800, 'MODE': 'A'}
                COMMON_3031AE   =  {'LOW': 15,'MID':60,     'HIGH':150,  'MODE': 'A'}
                COMMON_3032AE   =  {'LOW': 50,'MID':15,     'HIGH':500,  'MODE': 'A'}
                model_config = {
                                'PEL-3021A':COMMON_3021,'PEL-3021':COMMON_3021,'LSG-175A': COMMON_3021,
                                'PEL-3041A':COMMON_3041,'PEL-3041':COMMON_3041,
    ...
    def PEL_CV_Mea_Readback(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                PEL_MEAS_READBK=self.DUT.getMeasure_Voltage(delay_before=1)
    ...
    def PEL_CP_Accuracy(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                if strM.find('LOW')>=0:
                    ...
                    val = r.value(TH.col_ItemName_3t)
                    if val not in ("", None):
                        self.DUT.setPower_response_Mode(val)
                    psw.setVoltage(r.value(TH.col_Cond_2))
                    psw.setCurrent(r.value(TH.col_Cond_DUT))
                    psw_vol=psw.getVoltage()
                    print(f"{psw_vol}")
                ...
                self.DUT.setInput_State(1)
                self.txtMsg_append(f"DUT_LOAD_ON...")
                PEL_CP_SET_POWER = float(self.DUT.getCP_Power())
                DMM_CP_MEAS_V    = float(DMM.getMainReading(delay_before=1.5))
                PCS_CP_MEAS_A    = float(PCS.getReading_DCI(delay_before=1.5))
                PSW_CP_SET_V=psw.getVoltage()
                PEL_CP_MEAS      = float(PCS_CP_MEAS_A*DMM_CP_MEAS_V)
                
                COMMON_3021     =   {'fs': 1.75,  'VAR': 500000}
                COMMON_3041     =   {'fs': 3.5,   'VAR': 500000}
                COMMON_3111     =   {'fs': 10.5,  'VAR': 500000}
                COMMON_3021H    =   {'fs': 1.75,  'VAR': 3240000}
                COMMON_3041H    =   {'fs': 3.5,   'VAR': 2210000}
                COMMON_3111H    =   {'fs': 10.5,  'VAR': 2210000}
                COMMON_3031AE   =   {'fs': 300,   'VAR': 500000}
                COMMON_3032AE   =   {'fs': 300,   'VAR': 500000}
                ...
                for model_str, config in model_config.items():
                    if model_str in DUT_CP_MODE:
                        ...
                        VAR = config.get('VAR')
                        break  
                if fs is None or VAR is None:
                    raise ValueError(f"未知的 DUT_CP_MODE: {DUT_CP_MODE}")
                OPER_MODE = self.DUT.getCurrent_Range()
                FS_SCALE = {
                        'LOW': 1,
                        'MID': 100,
                ...
                #我這段程式需要再如果model_config是'PEL-3031AE'和'PEL-3032AE'的時候,就直接取值COMMON_3031AE   =   {'fs': 300,   'VAR': 500000} COMMON_3032AE   =   {'fs': 300,   'VAR': 500000}
                #fs_eff 不去做FS_SCALE 倍數的計算
                fs_eff = float(fs) * FS_SCALE.get(OPER_MODE, 1)
                if any(m in DUT_CP_MODE for m in ('PEL-3041', 'PEL-3021', 'PEL-3111','LSG-175A')):
                ...
                elif any(m in DUT_CP_MODE for m in('PEL-3021H','PEL-3041H','PEL-3111H','LSG-1050AH','LSG-175AH')):
                    PL= PEL_CP_SET_POWER - ((0.6 / 100) * PEL_CP_SET_POWER + (1.4 / 100) * fs_eff) + (DMM_CP_MEAS_V * DMM_CP_MEAS_V / VAR)
                    PU= PEL_CP_SET_POWER + ((0.6 / 100) * PEL_CP_SET_POWER + (1.4 / 100) * fs_eff) + (DMM_CP_MEAS_V * DMM_CP_MEAS_V / VAR)

                elif any(m in DUT_CP_MODE for m in('PEL-3031AE','PEL-3032AE')):
                    PL= PEL_CP_SET_POWER - ((0.6 / 100) * PEL_CP_SET_POWER + (1.4 / 100) * 300) + (DMM_CP_MEAS_V * DMM_CP_MEAS_V / VAR)
                    PU= PEL_CP_SET_POWER + ((0.6 / 100) * PEL_CP_SET_POWER + (1.4 / 100) * 300) + (DMM_CP_MEAS_V * DMM_CP_MEAS_V / VAR)
        ...
        try:
            ...
            serr=e        
        return sJudge
    def PEL_CC_IMON_Accuracy(self,strIName='',FixUse='all'):
        TH=recordHeader()
        sJudge=''
        try:
            #if self.DUT.instrConnected==0 or not(PSW.instrConnected==1):     #確認此項需要的設備是否有連接
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                self.DUT.setInput_State(1)
                self.txtMsg_append(f"DUT_LOAD_ON...")
                PCS_read=PCS.getReading_DCI(delay_before=1)
                PCS_VMO_ACC=PCS.getReading_DCV(delay_before=0.5)
                #DMM_CC_MEAS_V=DMM.getMainReading(delay_before=1.5)
        
                #COMMON_3021     =  {'LOW': 2.857142857, 'MID':0.028571429, 'HIGH': 0.028571429,    'MODE': 'A'}
                #COMMON_3041     =  {'LOW': 1.428571429, 'MID':0.014285714,  'HIGH': 0.014285714,   'MODE': 'A'}
                #COMMON_3111     =  {'LOW': 0.476190476, 'MID':0.004761905, 'HIGH': 0.004761905,   'MODE': 'A'}
                #COMMON_3021H    =  {'LOW': 0.114285714,'MID':1.142857143,    'HIGH':1.142857143,    'MODE': 'B'}
                #COMMON_3041H    =  {'LOW': 0.057142857, 'MID':0.571428571,   'HIGH':0.571428571,   'MODE': 'B'}
                #COMMON_3111H    =  {'LOW': 19.04761905,'MID':0.19047619,    'HIGH':0.19047619, 'MODE': 'B'}
                

                MODEL_CONFIG = {
                        'PEL-3021A':  {'COMMON': '3021'},'PEL-3021':  {'COMMON': '3021'},'LSG-175A':  {'COMMON': '3021'},
                        'PEL-3041A':  {'COMMON': '3041'},'PEL-3041':  {'COMMON': '3041'},
                        'PEL-3111A':  {'COMMON': '3111'},'PEL-3111':  {'COMMON': '3111'},
                        'PEL-3021H': {'COMMON': '3021H'},'PEL-3021AH': {'COMMON': '3021H'},'LSG-175AH':{'COMMON':'3021H'},
                        'PEL-3041H': {'COMMON': '3041H'},'PEL-3041AH': {'COMMON': '3041H'},
                        'PEL-3111H': {'COMMON': '3111H'},'PEL-3111AH': {'COMMON': '3111H'},  'LSG-1050AH':{'COMMON': '3111H'},
                                }
                COMMON_CONFIG = {
                        '3021': {
                            'FS': {'LOW': 2.857142857, 'MIDDLE': 0.028571429, 'HIGH': 0.028571429},
                            'MODE': 'A',
                        },
                        '3041': {
                            'FS': {'LOW': 1.428571429, 'MIDDLE': 0.014285714, 'HIGH': 0.014285714},
                            'MODE': 'A',
                        },
                        '3111': {
                            'FS': {'LOW': 0.476190476, 'MIDDLE': 0.004761905, 'HIGH': 0.004761905},
                            'MODE': 'A',
                        },
                        '3021H': {
                ...
                TOL_TABLE = {
                                'LOW':    {'TOL_1': 0.005, 'TOL_2': 0.005},     
                                'MIDDLE':    {'TOL_1': 0.007, 'TOL_2': 0.01},      
                                'HIGH':   {'TOL_1': 0.005, 'TOL_2': 0.005},     
                            }       
    ...
    def PEL_CV_VMON_Accuracy(self,strIName='',FixUse='all'):
        TH=recordHeader()
        sJudge=''
        try:
            #if self.DUT.instrConnected==0 or not(PSW.instrConnected==1):     #確認此項需要的設備是否有連接
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                if strM.find('LOW')>=0:
                    ...
                    self.DUT.setCurrent_Range("HIGH")
                    self.DUT.setVoltage_Range("HIGH")
                    self.DUT.setCV_Voltage(r.value(TH.col_Cond_1))
                    psw.setVoltage(r.value(TH.col_Cond_2))
                    psw.setCurrent(r.value(TH.col_Cond_DUT))
                    psw_vol=psw.getVoltage()
                ...
                self.DUT.setInput_State(1)
                self.txtMsg_append(f"DUT_LOAD_ON...")
                DMM_read=float(DMM.getMainReading(delay_before=1.5))
                PCS_read=PCS.getReading_DCI(delay_before=0.8)
                PCS_VMO_ACC=PCS.getReading_DCV(delay_before=0.5)
                #DMM_CC_MEAS_V=DMM.getMainReading(delay_before=1.5)
...
#① 開 VS Code
#② 開 Terminal，輸入：
#git clone https://github.com/你的帳號/my_python_project.git
#請main desk繼續更新
